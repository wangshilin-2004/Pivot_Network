...
$dockerContexts = Invoke-DockerCliCapture -Arguments @("context", "ls", "--format", "{{json .}}") -TimeoutSeconds 6
$dockerSwarm = Invoke-DockerCliCapture -Arguments @("info", "--format", "{{json .Swarm}}") -TimeoutSeconds 8
...
$dockerDesktopDaemonWireGuard = Get-DockerDesktopDaemonWireGuardSample `
  -Distro "docker-desktop" `
  -InterfaceName $HostTunnelName `
  -ManagerWireGuardAddress $ManagerWireGuardAddress `
  -TcpPorts @($ManagerSwarmPort, $SwarmGossipPort)
...
[PSCustomObject]@{
  ...
  docker_desktop = [ordered]@{
    ...
    daemon_wireguard = $dockerDesktopDaemonWireGuard
