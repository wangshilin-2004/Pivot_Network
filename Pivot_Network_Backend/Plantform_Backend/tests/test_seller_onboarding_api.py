...
def override_services(
    *,
    ...
    store = InMemoryStore()
    temp_dir = Path(tempfile.mkdtemp(prefix="seller-onboarding-db-"))
    engine = create_engine(
        f"sqlite+pysqlite:///{temp_dir / 'seller_onboarding.db'}",
    ...
    def seller_onboarding_override():
        session = session_local()
        try:
            yield SellerOnboardingService(SellerOnboardingRepository(session), adapter)
        finally:
            session.close()

    app.dependency_overrides[get_seller_onboarding_service] = seller_onboarding_override
    return adapter
...
def test_seller_onboarding_happy_path() -> None:
    ...
    try:
        ...
        assert payload["truth_authority"] == "raw_manager"
...
def test_correction_reverify_and_minimum_tcp_validation_close_the_truth_chain() -> None:
    ...
    try:
        ...
        assert reverify_payload["truth_authority"] == "raw_manager"
        ...
        assert tcp_payload["minimum_tcp_validation"]["truth_authority"] == "raw_manager"
...
def test_manager_address_override_exposes_effective_target_and_allows_tcp_validation_against_override() -> None:
    ...
    try:
        ...
        assert tcp_payload["minimum_tcp_validation"]["truth_authority"] == "backend_correction"
        assert tcp_payload["minimum_tcp_validation"]["detail"] is None
    finally:
        app.dependency_overrides.clear()


def test_authoritative_effective_target_establishes_backend_truth_and_tcp_validation_marks_session_verified() -> None:
    override_services(node_addr="202.113.184.2", expected_wireguard_ip="10.66.66.10")
    client = TestClient(app)

    try:
        headers = register_seller(client, email="seller-authoritative@example.com")

        create = client.post(
            "/api/v1/seller/onboarding/sessions",
            headers=headers,
            json={"requested_compute_node_id": "temp-seller-authoritative-node"},
        )
        assert create.status_code == 201, create.text
        session_id = create.json()["session_id"]
        ...
        join_complete = client.post(
            ...
            json={
                "reported_phase": "repair",
                "compute_node_id": "temp-seller-authoritative-node",
                "observed_wireguard_ip": "10.66.66.10",
                "observed_advertise_addr": "10.66.66.10",
                "observed_data_path_addr": "10.66.66.10",
            },
        ...
        assert mismatch_payload["manager_acceptance"]["status"] == "mismatch"
        assert mismatch_payload["effective_target_addr"] is None
        assert mismatch_payload["truth_authority"] == "raw_manager"

        authoritative = client.post(
            f"/api/v1/seller/onboarding/sessions/{session_id}/authoritative-effective-target",
            headers=headers,
            json={
                "reported_phase": "repair",
                "source_surface": "backend_authoritative_workflow",
                "effective_target_addr": "10.66.66.10",
                "effective_target_reason": "fresh runtime evidence plus manager raw mismatch",
        ...
        assert authoritative_payload["status"] == "verify_failed"
        assert authoritative_payload["manager_acceptance"]["status"] == "mismatch"
        assert authoritative_payload["effective_target_addr"] == "10.66.66.10"
        assert authoritative_payload["effective_target_source"] == "backend_correction"
        assert authoritative_payload["truth_authority"] == "backend_correction"
        assert authoritative_payload["correction_history"][-1]["correction_action"] == "backend_authoritative_correction"
        ...
        assert tcp_validation.status_code == 200, tcp_validation.text
        tcp_payload = tcp_validation.json()
        assert tcp_payload["status"] == "verified"
        assert tcp_payload["truth_authority"] == "backend_correction"
        assert tcp_payload["minimum_tcp_validation"]["validated_against_manager_target"] is False
        assert tcp_payload["minimum_tcp_validation"]["validated_against_effective_target"] is True
        assert tcp_payload["minimum_tcp_validation"]["effective_target_addr"] == "10.66.66.10"
        assert tcp_payload["minimum_tcp_validation"]["effective_target_source"] == "backend_correction"
        assert tcp_payload["minimum_tcp_validation"]["truth_authority"] == "backend_correction"
        assert tcp_payload["minimum_tcp_validation"]["detail"] is None
    finally:
        app.dependency_overrides.clear()
