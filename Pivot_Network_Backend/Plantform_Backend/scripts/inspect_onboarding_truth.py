...
def _session_truth(session_response: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "correction_history": session.get("correction_history") or [],
        "manager_acceptance": session.get("manager_acceptance"),
        "effective_target_addr": session.get("effective_target_addr"),
        "effective_target_source": session.get("effective_target_source"),
        "truth_authority": session.get("truth_authority"),
        "minimum_tcp_validation": session.get("minimum_tcp_validation"),
    }
...
def _derive_summary(report: dict[str, Any]) -> dict[str, Any]:
    ...
    minimum_tcp_validation = session.get("minimum_tcp_validation") if isinstance(session, dict) else None
    ...
    effective_target_addr = _clean_optional_string(session.get("effective_target_addr") if isinstance(session, dict) else None)
    effective_target_source = _clean_optional_string(session.get("effective_target_source") if isinstance(session, dict) else None)
    truth_authority = _clean_optional_string(session.get("truth_authority") if isinstance(session, dict) else None)
    ...
    if effective_target_addr and effective_target_addr == expected_wireguard_ip:
        signals.append("effective_target_matches_expected_wireguard_ip")
    if (
        isinstance(minimum_tcp_validation, dict)
        and minimum_tcp_validation.get("validated_against_effective_target")
        and minimum_tcp_validation.get("reachable")
    ):
        signals.append("minimum_tcp_validation_passed_against_effective_target")
    ...
    if truth_authority == "backend_correction" and "minimum_tcp_validation_passed_against_effective_target" in signals:
        interpretation = "backend_authoritative_target_validated"
    elif "manager_node_addr_mismatch" in signals:
